...
subtitle: "Type I and II error rates are not sufficient, but getting to error rates that matter require Bayesian-like assumptions"
format: 
  html:
    code-fold: show
...
We could skirt around this issue by reframing error rates to focus on limits/margins, similar to how it's done with MDE. However, a statement like "we have at most X% risk of sign error when we detect statistically significant results and the underlying effect size is at least Y" does not feel very satisfactory. Can we do better?
...
ggplot(avg_sign_error) + 
  ...
  geom_vline(xintercept = 0.8, linetype='dashed') +
  scale_y_continuous(breaks=seq(0, 0.016, 0.002))
...
Me neither. So I ran another simulation just to double check.
...
used_sample_size = required_sample_size
used_sample_size = 5000
...
appendix_sim = pblapply(eff_size_distrs, function(d) {
  ...
  exp_results = tibble(
    ...
    mutate(
      ...
      sign_error = if_else(is_stat_sig, sign(mean_diff) != sign(drawn_effect_sizes), NA),
  ...
  exp_results |> summarize(
    distribution = d$name,
    avg_power = mean(is_stat_sig, na.rm=T),
    avg_sign_error = mean(sign_error, na.rm=T),
    avg_exaggeration = mean(exaggeration, na.rm=T)
  )
}) |> bind_rows()


appendix_sim |> kbl(caption='Simulation results') |> kable_styling()
```

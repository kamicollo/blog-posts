...
-   Type I error rate only provides guarantees for experiments with zero impact. This is not very useful in the industry setting where the null hypothesis is rarely true and small effect sizes matter (from a business perspective, they are not equivalent to zero).

-   In the context of long-term error rates, Type S error (the probability of observing a result with an opposite sign than the actual effect) and Type M error (the ratio of exaggeration of observed effect vs. actual effect) are much more helpful measures than pure Type I error rates.
...
-   Overall, the long-term error/detection rate framework provides an alternative view to determining experiment sample sizes. It shows that the selection of MDE (and resulting statistical power) is less important than the relationship between effect size distribution and sample sizes, hopefully paving way to more productive discussions about MDE levels that should be used in large-scale A/B testing programs.
